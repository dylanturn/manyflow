...
async function handler(req: AuthenticatedRequest) {
  ...
  try {
    const { endpointId, dagIds, isPaused } = await req.json()
    
    if (!endpointId || !dagIds || typeof isPaused !== 'boolean') {
    ...
    const results = await DagService.batchToggleDagState(endpoint.url, dagIds, isPaused)
